exports.getCurrentBatters = function (payload) {
    ...
    const getBallsFacedForPlayer = function(periodStat) {
        return periodStat.statistics.find(stat => {
            return (stat.type.type === "BATTING" && stat.type.value === "BALLS_FACED")
        }).count
    }
    ...
    return notOutPlayers.map(function(item) {
        return {
            ...
            ballsFaced: getBallsFacedForPlayer(item.periodStats)
        }
    });
}